...
from app.models import AiPattern, Campaign, CampaignVariation, EmailSequence, EmailTemplate, Lead
from app.schemas import (
    ...
    CampaignUpdate,
    CampaignVariationResponse,
    MessageResponse,
)
from app.utils.ai_service import analyze_ab_test, generate_email_variations
...
async def get_campaigns(
    ...
    status: str | None = Query(None, pattern="^(draft|scheduled|active|completed|paused)$"),
...
async def get_campaign(campaign_id: int, db: AsyncSession = Depends(get_db)):
    ...
    # TODO: In production, link EmailSequence to Campaign via FK
    # and use:
    #   select(func.count(EmailSequence.id), ...)
...
async def update_campaign(campaign_id: int, campaign_data: CampaignUpdate, db: AsyncSession = Depends(get_db)):
    ...
    if campaign_data.status and campaign.status == "active":
        raise HTTPException(
            status_code=400, detail="Use /pause or /complete endpoints to change active campaign status"
        )
...
async def start_campaign(campaign_id: int, request: CampaignStartRequest, db: AsyncSession = Depends(get_db)):
    """
    ...
    if request.email_template_id:
        template_result = await db.execute(select(EmailTemplate).where(EmailTemplate.id == request.email_template_id))
        template = template_result.scalar_one_or_none()

        if not template:
            raise HTTPException(status_code=404, detail=f"Email template with ID {request.email_template_id} not found")

        subject = template.subject
        body = template.body

    if not subject or not body:
        raise HTTPException(status_code=400, detail="Provide email_template_id OR both subject and body")

    # Get leads
    leads_result = await db.execute(select(Lead).where(Lead.id.in_(request.lead_ids)))
    leads = leads_result.scalars().all()

    if not leads:
        raise HTTPException(status_code=400, detail="No valid leads found with provided IDs")
...
async def pause_campaign(campaign_id: int, db: AsyncSession = Depends(get_db)):
    ...
    if campaign.status != "active":
        raise HTTPException(status_code=400, detail=f"Campaign is not active (current status: {campaign.status})")
...
async def complete_campaign(campaign_id: int, db: AsyncSession = Depends(get_db)):
...
async def resume_campaign(campaign_id: int, db: AsyncSession = Depends(get_db)):
    ...
    if campaign.status != "paused":
        raise HTTPException(status_code=400, detail=f"Campaign is not paused (current status: {campaign.status})")
...
async def get_campaign_emails(
    ...
    status: str | None = Query(None, pattern="^(pending|scheduled|sent|opened|replied|bounced)$"),
    db: AsyncSession = Depends(get_db),
):
    """
    ...
    leads_map = {ld.id: ld for ld in leads_result.scalars().all()}
...
async def quick_start_campaign(
    ...
    template_id: int | None = Query(None, description="Email template ID"),
    subject: str | None = Query(None, description="Custom subject (if no template)"),
    body: str | None = Query(None, description="Custom body (if no template)"),
    db: AsyncSession = Depends(get_db),
):
    """
    ...
    """
    # Create campaign
    campaign = Campaign(name=name, status="draft")
    db.add(campaign)
    await db.commit()
    await db.refresh(campaign)
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail="Invalid lead_ids format. Use comma-separated integers.")

    # Build start request
    start_request = CampaignStartRequest(
        lead_ids=parsed_lead_ids, email_template_id=template_id, subject=subject, body=body
    )
...
async def generate_campaign_emails(
    ...
    """
    Generate 5 AI email variations for a campaign.

    The LLM reads learned patterns from past A/B tests and generates
    5 variations, each targeting a different psychological trigger.
    """
    result = await db.execute(select(Campaign).where(Campaign.id == campaign_id))
    campaign = result.scalar_one_or_none()
    if not campaign:
    ...
    patterns_result = await db.execute(
        select(AiPattern.pattern)
        .order_by(AiPattern.created_at.desc())
        .limit(10)
    )
    learned_patterns = [row[0] for row in patterns_result.fetchall()]

    ai_result = await generate_email_variations(
        brief={
            "target_audience": brief.target_audience,
            "goal": brief.goal,
            "pain_point": brief.pain_point or "low engagement",
            "tone": brief.tone,
    ...
    for v in ai_result.get("variations", []):
        cv = CampaignVariation(
            campaign_id=campaign_id,
            label=v["label"],
            subject=v["subject"],
            body=v["body"],
            psychological_trigger=v.get("psychological_trigger"),
        )
        db.add(cv)
        await db.flush()
    ...
    return CampaignGenerateResponse(
        campaign_id=campaign_id,
        variations=[
            {"label": v["label"], "subject": v["subject"], "body": v["body"], "psychological_trigger": v["psychological_trigger"]}
            for v in variations_out
        ],
        patterns_used=ai_result.get("patterns_used", 0),
    )


# ============================================
# LIST VARIATIONS WITH STATS
# ============================================
@router.get("/{campaign_id}/variations", response_model=list[CampaignVariationResponse])
async def get_campaign_variations(
    campaign_id: int,
    db: AsyncSession = Depends(get_db),
):
    """Get all AI-generated variations for a campaign with engagement stats."""
    result = await db.execute(select(Campaign).where(Campaign.id == campaign_id))
    if not result.scalar_one_or_none():
        raise HTTPException(status_code=404, detail=f"Campaign {campaign_id} not found")

    vars_result = await db.execute(
        select(CampaignVariation)
        .where(CampaignVariation.campaign_id == campaign_id)
        .order_by(CampaignVariation.label)
    )
    variations = vars_result.scalars().all()

    out = []
    for v in variations:
        open_rate = (v.opens / v.sends * 100) if v.sends else 0.0
        reply_rate = (v.replies / v.sends * 100) if v.sends else 0.0
        out.append(CampaignVariationResponse(
            id=v.id,
            label=v.label,
            subject=v.subject,
            body=v.body,
            psychological_trigger=v.psychological_trigger,
            sends=v.sends,
            opens=v.opens,
            clicks=v.clicks,
            replies=v.replies,
...
# A/B TEST ANALYSIS
# ============================================
@router.post("/{campaign_id}/analyze", response_model=ABTestWinner)
async def analyze_campaign_ab_test(
    campaign_id: int,
    db: AsyncSession = Depends(get_db),
):
    """
    ...
    The LLM weighs replies (50%), opens (30%), and clicks (20%) to pick
    a winner. The winning pattern is saved to ai_patterns for future use.
    """
    result = await db.execute(select(Campaign).where(Campaign.id == campaign_id))
    campaign = result.scalar_one_or_none()
    if not campaign:
    ...
    if not variations:
        raise HTTPException(status_code=400, detail="No variations found for this campaign. Generate them first.")
    ...
    ai_result = await analyze_ab_test(variations_data)

    winner_label = ai_result.get("winner_label", "A")
    winner_var = next((v for v in variations if v.label == winner_label), variations[0])

    for v in variations:
        v.is_winner = (v.label == winner_label)

    pattern_text = ai_result.get("pattern_learned", "")
    if pattern_text:
        total_sends = sum(v.sends for v in variations)
    ...
    stats = {v.label: {"sends": v.sends, "opens": v.opens, "clicks": v.clicks, "replies": v.replies} for v in variations}

    return ABTestWinner(
        campaign_id=campaign_id,
        winner_label=winner_label,
        winner_subject=winner_var.subject,
        winner_body=winner_var.body,
        explanation=ai_result.get("explanation", ""),
        pattern_learned=pattern_text,
        stats=stats,
    )
